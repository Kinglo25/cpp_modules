...
/*   Updated: 2022/05/30 19:45:27 by lmajerus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(Form const &in)
{
	try
	{
		in.execute(*this);
		std::cout << this->get__name() << " executed " << in.get__name() << std::endl;
	}
	catch (const std::exception &e)
	{
		std::cout << this->get__name() << " couldn't execute "
				  << in.get__name() << " because " << e.what()
				  << std::endl;
	}
}
